...
use GraphQL\Collections\Data\NftMetaData;
use GraphQL\Collections\Data\NftMetaAttribute;
use DateTime;
use Slogger;
...
//TODO: Must set to timezone of server location
date_default_timezone_set('Europe/Zurich').

$class = "DataSource";

//TODO: move to config file 
$collectionSource = "PRD";
$eventSource = "MOCK";
$nftSource = "PRD";
...
class DataSource
{
    ...
    public static function findCollection(int $id): ? Collection
    {
        ...
        if($collectionSource != "MOCK" && $id > 0){
            $query = "
                SELECT 
                    t.id, tokenname, t.tokenid, description 
                FROM token_handler t  
                JOIN descriptions d  ON t.tokenid = d.tokenid 
                WHERE id = ".$id."
            ";
            $slogger->debug($class , "Quering: ".$query);
            try {
                ...
                $slogger->debug($class , "ERROR: impossible to perform the query ".$th);
            }
            //$slogger->debug($class , "checking response");
            ...
            if($res != null) {
                ...
                $new = new Collection([
                    'name' => $res["tokenname"],
                    'id' => $id, 
                    'contract' => $res["tokenid"],
                    'img' => "",
                    'description' => $res["description"]
                ]);
                self::initCollectionFields($new);
                return $new;
            } else {
                $slogger->debug($class , "ALERT: Token not found");
            }
    ...
    public static function initCollectionFields(Collection $c): ? Collection
    {
        global $slogger, $class;
        foreach (self::$collections[0] as $key => $value) {
            $slogger->debug($class , "check ".$key);
            if(!isset($c->$key)){
                $slogger->debug($class , "y ".$key);
                $c->$key = $value;
            } else {
                $slogger->debug($class , "n ".$key);
            }
        }
    ...
    {
        global $nftSource, $pdo;

        if ($nftSource != "MOCK") {
            $query = "SELECT * FROM nfts WHERE token_address = '" . $token_address . "' and token_id = '" . $token_id . "' and TO_TIMESTAMP(last_update,'YYYY-MM-DD HH24:MI:SS') >= NOW() - INTERVAL '24 HOURS' ORDER BY last_update DESC limit 1 ";
            $stmt = $pdo->query($query);
            $res = $stmt->fetch();

            //TODO: Found in database and the data is fresh  
            if ($res) {
    ...
    {
        global $nftSource, $pdo, $slogger, $class, $mockType;

        $resArray = array();

        if ($nftSource != "MOCK") {
            $slogger->debug($class, "findNfts start for collection: ".$token_address);

            $query = "
                SELECT t1.* FROM nfts t1 
                JOIN ( 
                    SELECT token_address, MAX(last_update) AS MAXDATE 
                    FROM nfts GROUP BY token_address
                ) t2 ON 
                    T1.token_address = t2.token_address 
                    AND t1.token_address='" . $token_address . "' 
                    AND t1.last_update = t2.MAXDATE
                ";
            $slogger->debug($class, "Executing: " . $query);
            try {
                $stmt = $pdo->query($query);
                $res = $stmt->fetch();
            } catch (\Throwable $th) {
                $slogger->error($class, "ERROR executing query");
            }

            // TODO: Get total time difference between two dates
            try {
                $currentDate = new DateTime();
                $lastUpdate = new DateTime($res['last_update'] ?? '');
                $diffTime = date_diff($currentDate, $lastUpdate)->format('%h');
            } catch (\Throwable $th) {
                $slogger->error($class, "ERROR trasforming data");
            }
            

            //TODO: Found in database and the data is fresh  
            if ($res && ((int) $diffTime < 24)) {
                $slogger->debug($class, "Collection retrieved from DB -> diffTime:" . $diffTime);
                foreach ($pdo->query($query) as $row) {
                    array_push($resArray, self::populateNft($row));
                }
            } else {
                $slogger->debug($class, "Collection NOT found, calling Moralis");
                $owner_nfts = self::fetchNftsfromMoralis(token_address: $token_address);
                foreach ($owner_nfts as $value) {
                    self::storeNftAtDatabase($value);
                    array_push($resArray, self::newNft($value) );
                }
                $slogger->debug($class, "NFTs Stored to db and added to result. check: resArray size: ".count($resArray));
            }
            /**
             *  TODO: the response is always empty, probably becouse the type returned is not compliant with the 
             *  one expected. 
             */
    ...
    public static function newNft($value) : ?Nft {
        global $slogger, $class;
        //TODO: to be filled with $value data
        $new = new Nft([
            'token_address' => "0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB",
            'token_id' => '15',
            'owner_of' => '0x9c83ff0f1c8924da96cb2fcb7e093f78eb2e316b',
    ...
    public static function fetchNftsfromMoralis(string $address = '0xb47e3cd837dDF8e4c57F05d70Ab865de6e193BBB', string $token_address = NULL, int $limit = NULL): ?array
    {
        global $nftSource, $pdo, $slogger, $class, $mockType;

        $slogger->debug($class, "fetchNftsfromMoralis - START - collection: ".$token_address);
        $curl = curl_init();
        $curlopt_url = "https://deep-index.moralis.io/api/v2/" . $token_address . "/nft?chain=eth&format=decimal&normalizeMetadata=true";
        $curlopt_url .= is_null($token_address) ? "" : "&token_addresses=" . $token_address;
        $curlopt_url .= is_null($limit) ? "" : "&limit=" . $limit;
        ...
        CURLOPT_HTTPHEADER => [
                "X-API-Key: 5xvLZsxDHu6MbFDzLfg7QcSSglQIFDJBMeHcHCVzRy4b28hOHC4Tp7H3Toupo2BP",
                "accept: application/json"
            ],
        ]);

        $slogger->debug($class, "calling: ".$curlopt_url);

        $response = curl_exec($curl);
        //$slogger->debug($class, "response: ".$response);

        $err = curl_error($curl);
        //$slogger->debug($class, "Errors: ".$err);

        $response = json_decode($response, true);

        curl_close($curl);

        return $err ? NULL : ($response['result'] ?? NULL);
    }

    public static function getMetaAttribute($val)
    {
        return new NftMetaAttribute([
            "trait_type" => $val['trait_type'],
            "value" => $val['value'],
            "display_type" => $val["display_type"] ?? '',
            "max_value" => $val['max_value'] ?? 0,
            "trait_count" => $val['trait_count'],
            "order" => $val['order'] ?? 0
        ]);
    }
    ...
    public static function storeNftAtDatabase($row)
    {
        ...
        try {
            $stmt = $pdo->query($query);
            $slogger->debug($class, "NFT to DB added");
        } catch (\Throwable $th) {
            $slogger->error($class, "FAILED to INSERT NFT to DB: ".$query);
        }
    ...
    public static function updateNftAtDatabase($row)
    {
        global $pdo;
        $nowDate = (new DateTime())->format('Y-m-d H:i:s');
        $query = "update nfts set token_address ='" . $row['token_address'] . "',token_id='" . $row['token_id'] . "',owner_of='" . $row['owner_of'] . "',token_hash='" . $row['token_hash'] . "',block_number='" . $row['block_number'] . "',block_number_minted='" . $row['block_number_minted'] . "',contract_type='" . $row['contract_type'] . "',token_uri='" . $row['token_uri'] . "',m_name='" . $row['normalized_metadata']['name'] . "',m_description='" . $row['normalized_metadata']['description'] . "',m_image='" . $row['normalized_metadata']['image'] . "',m_external_link='" . $row['normalized_metadata']['external_link'] . "',m_animation_url='" . $row['normalized_metadata']['animation_url'] . "',m_attributes='" . json_encode($row['normalized_metadata']['attributes']) . "',minter_address='" . $row['minter_address'] . "',last_token_uri_sync='" . $row['last_token_uri_sync'] . "',last_metadata_sync='" . $row['last_metadata_sync'] . "',amount='" . $row['amount'] . "',name='" . $row['name'] . "',symbol='" . $row['symbol'] . "',last_update='" . $nowDate . "' where token_address = '" . $row['token_address'] . "' and token_id = '" . $row['token_id'] . "'";
        $stmt = $pdo->query($query);
    }

    public static function populateNft($row)
    {
        $m_attributes = isset($row['m_attributes']) ? json_decode($row['m_attributes'], true) : [];
        ...
        $result = new Nft([
            ...
            'normalized_metadata' => new NftMetaData([
                'name' => $row['normalized_metadata']['name'] ?? ($row['m_name'] ?? ''),
                'description' => $row['normalized_metadata']['description'] ?? ($row['m_description'] ?? ''),
                'image' => $row['normalized_metadata']['image'] ?? $row['m_image'],
                'external_link' => $row['normalized_metadata']['external_link'] ?? ($row['m_external_link'] ?? ''),
                'animation_url' => $row['normalized_metadata']['animation_link'] ?? ($row['m_animation_link'] ?? ''),
                'attributes' => array_map('self::getMetaAttribute', ($row['normalized_metadata']['attributes'] ?? $m_attributes))
            ]),
